...
- **Plan 01-01 — Foundation (this skeleton):** pnpm + Next.js 16 + Tailwind 4 + shadcn scaffold; tooling configs; Zod `WorkoutSchema` lock; service factory + MockExtractionService + RealExtractionService stub; SSE route handler; baseline `dumbbell-leg-day.json` fixture; AmbientBackground + Footer chrome; Tailwind 4 `@theme` block + glass recipe + reduced-motion master block in globals.css; Vercel project setup (user_setup checkpoint). No interactive UI yet. (Split from original Plan 1 per checker B1.)
- **Plan 01-02 — Skeleton UI + First Deploy:** UrlInput, ExtractFlow (Client island with useReducer + SSE consumer + share-link hydration), LoadingStages, WorkoutView, ExerciseCard, SupersetCard, ActionBar, share encode/decode against `SharePayloadSchema` wrapper (W6 — wire format defined upfront with `stripped: []` no-op, stable across Phase 1), clipboard markdown + plaintext, README, local human verify, first Vercel deploy. (Split from original Plan 1 per checker B1.)
- **Plan 01-03 — Fixture variety + Error/empty/limit states:** Add 4 remaining fixtures (`bodyweight-push.json`, `full-body-2-supersets.json`, `warmup-3-exercises.json`, `hypertrophy-12-exercises.json`); enable deterministic `hash(videoId) mod fixtureCount` selection (D-12); wire URL-keyword error triggers in MockExtractionService (D-13); ship `<ErrorState />` variants for NETWORK/NO_WORKOUT/RATE_LIMITED with copy from UI-SPEC §7.4; full inline URL-validation error from UI-SPEC §7.1.
- **Plan 01-04 — Save & Share completeness (strip chain only):** Implement the D-17 strip chain (`sourceQuote → form_cues → equipment`) loop body in `lib/share/encode.ts` (the `SharePayloadSchema` wrapper itself already ships in Plan 01-02 per W6 — Plan 01-04 only changes encode-internals, not wire format); surface the "Share link omits form cues for length." notice on toast + on hydrated view (UI-SPEC §11); polish the D-18 schema-version error path through `<ErrorState />`; CI measurement task for compressed payload sizes across all 5 fixtures.
- **Plan 01-05 — Polish, A11y, Mobile, Final exit gate:** Mobile glass fallback recipe (UI-SPEC §5.2); full reduced-motion compliance across all 7 motion moments (UI-SPEC §6.5); axe-core remediation loop on deployed gradient until zero color-contrast violations (DSGN-06); sticky-bottom action bar on mobile with safe-area inset (OUTV-07); tooltip disambiguation on Copy buttons (UI-SPEC §8.1); cross-device human share-link smoke test on the frozen deploy; STATE.md + ROADMAP.md finalization. Task 3 split into 3a/3b/3c per checker B3 so the axe-core remediation loop, human cross-device gate, and state finalization are separate single-purpose tasks.
